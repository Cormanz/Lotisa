...
pub fn eval_board(board: &Board, moving_team: i16) -> i32 {
    let material: i32 = board.pieces.iter().map(|pos| {
...
pub fn negamax_root(board: &mut Board, moving_team: i16, depth: i16) -> EvaluationScore {
    let mut evaluation = negamax(board, moving_team, depth, -2147483647, 2147483647);
    //evaluation.score *= -1;
    evaluation
}

pub fn negamax(board: &mut Board, moving_team: i16, depth: i16, mut alpha: i32, beta: i32) -> EvaluationScore {
    if depth == 0 { 
    ...
    for action in board.generate_legal_moves(moving_team)  {
        let undo = board.make_move(action);
        let mut evaluation = negamax(board, if moving_team == 0 { 1 } else { 0 }, depth - 1, -beta, -alpha);
        evaluation.score *= -1;
        board.undo_move(undo);
