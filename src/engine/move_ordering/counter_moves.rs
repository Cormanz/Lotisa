...
pub fn store_counter_move(search_info: &mut SearchInfo, prev_action: Action, action: Action, depth: i16) {
...
pub fn is_counter_move(search_info: &mut SearchInfo, prev_action: &Action, action: &Action) -> bool {
    let to = prev_action.to as usize;
    let from = prev_action.from as usize;
