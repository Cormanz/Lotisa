...
fn en_passant_uci_test() {
    ...
    */
    ...
    assert!(!uci.board.generate_legal_moves().iter().any(|action| action.piece_type == 0 && action.info == -3), "There is an en passant in this position when there shouldn't be.");
}
